const ProviderButton = ({ provider, count }: { provider: string, count: number }) => {
    return (
        <button
            key={provider}
            className="shrink-0 flex items-center justify-between gap-4 px-4 py-3 bg-white border border-slate-100 rounded-xl shadow-sm hover:shadow-md hover:border-blue-200 transition-all min-w-180px"
        >
            <img
